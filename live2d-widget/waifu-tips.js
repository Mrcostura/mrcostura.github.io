...
!function() {
    ...
    class s {
		...
		async loadUrl() {
            open(localStorage.getItem("modelUrl"))
        }
        async loadModelList() {
        ...
        async loadRandModel() {
            ...
            if (this.useCDN) {
                ...
                o("¿Te gusta mi ropa nueva?", 4e3, 10)
            } else
                fetch(`${this.apiPath}rand_textures/?id=${t}-${s}`).then((e=>e.json())).then((e=>{
                    1 !== e.textures.id || 1 !== s && 0 !== s ? this.loadModel(t, e.textures.id, "¿Te gusta mi ropa nueva?") : o("¡Todavía no tengo otra ropa!", 4e3, 10)
                }
                ))
        }
        async loadOtherModel() {
            ...
            if (this.useCDN) {
				...
				localStorage.setItem("modelUrl",this.modelList.modelUrl[t])
            } else
    ...
    const n = {
        ...
        photo: {
            ...
            callback: ()=>{
                o("¿Está bien la foto? ¿No es muy linda?", 6e3, 9),
        ...
        quit: {
            ...
            callback: ()=>{
                ...
                o("Que un día puedas reunirte con las personas importantes en tu vida.", 2e3, 11),
    ...
    function i(t) {
        ...
        function c(t) {
            ...
            o(function(e) {
                ...
                const t = `Bienvendio a<span>「${document.title.split(" - ")[0]}」</span>`;
                let o;
                if ("" !== document.referrer) {
                    ...
                    `Hello！Amigo de <span>${o}</span><br>${t}`)
                }
                return t
            }(t.time), 7e3, 11),
        ...
        function() {
			...
			if (window.Live2DModel) {
				...
				.then(data => {
        // 在这里处理获取到的JSON数据
		let searchString = window.Live2DModel; // 你要查找的字符串
        let position = data.pathName.indexOf(searchString);
        if (position !== -1) {
			console.log(data.pathName[position]);
                i.loadModel(position, 53)
            }
	...
	let e = localStorage.getItem("modelId")
			...
			fetch("/live2d_models/model_list.json")
				...
				.then(data => {
        // 在这里处理获取到的JSON数据
		const listLength = data.pathName.length;
        const randomIndex = Math.floor(Math.random() * listLength);
        localStorage.setItem("modelUrl", data.modelUrl[randomIndex])
		console.log(data.pathName[randomIndex]);
        i.loadModel(randomIndex, 53)
